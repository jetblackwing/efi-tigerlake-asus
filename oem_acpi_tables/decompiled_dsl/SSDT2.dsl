...
DefinitionBlock ("", "SSDT", 2, "SaSsdt", "SaSsdt ", 0x00003000)
{
    ...
    External (_SB_.PC00.LPCB.EC0_.CECD, MethodObj)    // 1 Arguments
    ...
    If (((HGMD == 0x02) && (HGST == One)))
    {
        ...
        Scope (\_SB.PC00)
        {
            ...
            Method (PGON, 1, Serialized)
            {
                ...
                PFUN = GFUN (PION)
                PGSC (Arg0, One)
                If ((CCHL (PION, One) == Zero))
                {
                    Return (Zero)
                }

                \_SB.PC00.LPCB.EC0.WEBC (0x14, 0x02, Buffer (0x02)
                    {
                         0x00, 0x01                                       // ..
                    })
                Sleep (DLPW)
                Local0 = Zero
                While ((CCHL (PION, Zero) == Zero))
                {
                    If ((Local0 > 0x04))
                    {
                        Return (Zero)
                ...
                \_SB.PC00.LPCB.EC0.WEBC (0x07, Zero, Zero)
                Return (Zero)
            }
            ...
            Method (PGOF, 1, Serialized)
            {
                ...
                DIWK (PIOF)
                \_SB.PC00.LPCB.EC0.WEBC (0x08, Zero, Zero)
                Return (Zero)
            }
            ...
            Method (CCHL, 2, NotSerialized)
            {
                ...
                If ((\_SB.PC00.LPCB.EC0.CECD (Arg1) == Zero))
                {
                    Return (Zero)
                }
